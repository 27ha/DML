...
# 文件存放路径
path = '/home/wha/DML_code/DDP/data/train/'
...
# 查看最小数据的条数, iphone6s_1.npy: 22065 * 8192
dataPath = path + dataName_list[5]
...
for dataName in dataName_list:
    ...
    temp_data = np.load(data_path)[0:min_length]
    ...
    temp_label += 1
n = len(data_list)
train_data = data.TensorDataset(torch.tensor(np.array(data_list[:int(0.8*n)])), torch.tensor(np.array(label_list[:int(0.8*n)])))
val_data = data.TensorDataset(torch.tensor(np.array(data_list[int(0.8*n):])), torch.tensor(np.array(label_list[int(0.8*n):])))
